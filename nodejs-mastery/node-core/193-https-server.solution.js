...
/**
 * Exercise 193: HTTPS Server - Creating HTTPS Server with SSL - SOLUTION
 */
...
export function createJsonHttpsServer() {
  return https.createServer({
    ...
    res.end(JSON.stringify({ secure: true, message: 'HTTPS response' }));
  });
}
...
export function createCustomPortHttpsServer(port) {
  return https.createServer({
    ...
    res.end(`HTTPS Server on port ${port}`);
  });
}
...
export function createRedirectServer() {
  const httpsServer = https.createServer({
    ...
    res.end('Secure HTTPS Content');
  });

  const httpServer = http.createServer((req, res) => {
    res.writeHead(301, {
      'Location': `https://localhost:4443${req.url}`
    });
    res.end();
  });
...
// Tests
describe('Exercise 193: HTTPS Server - Solution', () => {
